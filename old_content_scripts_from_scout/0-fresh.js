...
const htmlGroup = (width) => {
  ...
  firstDomNodes.appendChild(window.scoutIframe = createElement('iframe', {
    ...
    style: rightPaneBaseStyles +
      'user-select:none; top:31px; height:100%; width:'+(100 - width)+'%;' +
      'background:white; border:none;'
  }))


  /////////////////////////////////
  //For updating tab/page title with sub-iframe page title
  //AWESOME but going to disable for now..
  ...
  //       })
  //     }
  //     console.log((message ? 'javascript window message:"'+message+'"' : 'empty(falsey)') + ' message from ' + event.origin + ':', event)
  //   }
  // }, false)
  // const updateTabDisplay = (data) => {
  ...
  // firstDomNodes.appendChild(title = createElement('title', {
  //   innerText: ' '
  // }))
...
body = () => {
  ...
  //visible elements
  SideBarShadowBaseStyles = 'position:fixed; display:block; left:0px;' +
    'height:128%; width:'+width+'%; background:transparent;' +
    'border:none;z-index: 113;pointer-events: none;'
  ...
  ///////// (fidgetting with the height of the page, probably for the topBar)
  //<STYLE>sidebar and right pane heights
  // secondDomNodes.appendChild(window.paneHeights = createElement('style', {
  //   innerText:
  //     'html>body{height: ' +
  //       (window.innerHeight -
  //         getComputedStyle(document.body)['margin-top']
  //       ) + 'px !important',
  //   id: 'paneHeights'
  // }))
