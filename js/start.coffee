isWebSearchUrl = (url) => {
  ...
  bailParams = ['&tbs=sbi:' '?tbs=sbi:'] # tbs is for searching by inputting an image, either dragging to a images.google.com tab or uploading.
  # [TECHNICALLY] the tbs param is base64 encoded image data.
  bailParams.forEach((param) => {
  ...
  ['isch', 'shop', 'nws', 'vid', 'plcs', 'dsc', 'rcp', 'app', 'pts', 'blg', 'bus'
  ].forEach((search) => {
